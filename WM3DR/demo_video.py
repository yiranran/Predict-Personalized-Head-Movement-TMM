...
def opts():
  ...
  parser.add_argument('--output', default='result0', type=str)
...
def main(opt):
  ...
  base = os.path.basename(opt.video_path)[:-4]
  ...
  with torch.no_grad():
    for i, image in enumerate(get_frames(opt.video_path)):
      ...
      outfile = os.path.join(opt.output, base, '{}.png'.format(str(i).zfill(8)))
      maskfile = os.path.join(opt.output, base, '{}_mask.png'.format(str(i).zfill(8)))
      renderfile = os.path.join(opt.output, base, '{}_render.png'.format(str(i).zfill(8)))
