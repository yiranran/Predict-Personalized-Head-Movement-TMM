...
def get_frames(video_name):
  if not video_name:
    ...
    if 'frame' in os.path.basename(images[0]):
      images = sorted(images, key=lambda x: int(x.split('/')[-1].split('.')[0][5:]))
    else:
...
def preprocess(image, res, device):
  h, w, _ = image.shape
  ...
  meta = {'s': max(h, w) / res}
  return torch.from_numpy(image).permute(2, 0, 1).unsqueeze(0).cuda(device), meta

def construct_meshes(shape, texture, face, device):
  nV = shape.size(2)
  ...
  for i in range(len(shape)):
    ...
    F_ = face.expand(V_.size(0), -1) + range_.cuda(device)
