...
class PtRender(nn.Module):
  def __init__(self, opt):
    ...
    shader_rgb = HardFlatShader(blend_params=blend_params,device=opt.device)

    self.renderer = Renderer(rasterizer, shader_rgb, SoftSilhouetteShader(), cameras, opt.device)
  ...
  def decode_translation(self, translation, index):
    ...
    trans = torch.zeros(translation.size()).cuda(self.opt.device)
  ...
  def Illumination_layer(self, face_texture, norm, gamma):
    ...
    if gamma.is_cuda: Y0 = Y0.cuda(self.opt.device)
    norm = norm.view(-1, 3)
