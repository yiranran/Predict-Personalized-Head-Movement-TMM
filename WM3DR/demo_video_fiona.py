...
def main(opt):
  ...
  if opt.base == '':
    base = os.path.basename(opt.video_path).split('.')[0]
  else:
    base = opt.base

  if not os.path.exists(opt.output):
    os.makedirs(opt.output)
  if not os.path.exists(os.path.join(opt.output, base)):
    print('syntest path', os.getcwd(), 'output', os.path.join(opt.output, base))
    os.makedirs(os.path.join(opt.output, base))

  with torch.no_grad():
    for i in range(len(glob.glob('{}/frame*.png'.format(opt.video_path)))):

      outfile = os.path.join(opt.output, base, '{}.png'.format(str(i).zfill(5)))
      maskfile = os.path.join(opt.output, base, '{}_mask.png'.format(str(i).zfill(5)))
      renderfile = os.path.join(opt.output, base, '{}_render.png'.format(str(i).zfill(5)))
      matfile = os.path.join(opt.output, base, '{}.mat'.format(str(i).zfill(5)))

      if os.path.exists(matfile) and os.path.exists(outfile) and os.path.exists(renderfile) and os.path.exists(maskfile):
        continue

      image = cv2.imread('{}/frame{}.png'.format(opt.video_path,i))
      h, w, _ = image.shape
      ...
      cv2.imwrite(maskfile, (gpu_masks * 255).astype(np.uint8))
      cv2.imwrite(renderfile, (rendered[..., ::-1] * 255).astype(np.uint8))
