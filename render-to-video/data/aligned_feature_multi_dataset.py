...
class AlignedFeatureMultiDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if 'wm3dr' not in self.opt.dataroot:
            frameno = int(frame[5:])
        ...
        if self.opt.model == 'memory_seq_2i':
            frameno_rands = list(range(0,300))
            random.shuffle(frameno_rands)
            ind = 0
            while frameno_rands[ind] == frameno or not os.path.exists(AB_path2.replace(frame,'frame%d'.format(frameno_rands[ind]))):
                ind += 1
            path2 = AB_path2.replace('frame%d'%(frameno),'frame%d'%(frameno_rands[ind]))
            print(AB_path2,path2) # only replace framexxx part in AB_path2 (from trainB), [34_nobg]frame3_input2.png->frameX_input2.png, [35_nobg]frame3.png->frameX.png
        ...
        if self.opt.isTrain or self.opt.test_use_gt:
            ...
            ss = AB_path2.split('/')
            if self.opt.dataroot != '300vw_win3' and self.opt.dataroot != 'lrwnewrender_win3' and ss[-3] != '19_news' and 'wm3dr' not in self.opt.dataroot:
                ...
                B_feat = np.load(os.path.join(self.opt.iden_feat_dir,ss[-3],ss[-2],ss[-1][:-4]+'.npy'))
            elif 'wm3drfull' in self.opt.dataroot:
                B_feat = np.load(os.path.join(self.opt.iden_feat_dir,ss[-3],ss[-2],ss[-1][:-4]+'.npy'))
            elif 'wm3dr' in self.opt.dataroot:
                B_feat = np.load(os.path.join(self.opt.iden_feat_dir,'Data',ss[-2],ss[-1][:-4]+'.npy'))
            item['B_feat'] = B_feat
