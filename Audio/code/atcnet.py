            ...
            if config.pose == 0:
                ...
                if config.dataset == 'lrw':
                    ...
                    self.dataset = News_1D_lstm_3dmm_pose(config.dataset_dir, train=config.is_train, indexes=config.indexes, relativeframe=config.relativeframe,
                                newsname=config.newsname, start=config.start, trainN=config.trainN, testN=config.testN, coef_fix=config.coef_fix)
...
def parse_args():
    ...
    # use coef_fix for coefs
    parser.add_argument('--coef_fix', type=bool, default=False)
